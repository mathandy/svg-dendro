...
def pasteableRing(ring):
    return "Ring('%s', '%s', '%s', Radius(%s), parse_path('%s'))" % (ring.string, ring.color, ring.brook_tag, ring.center, ring.string)
...
def isNotTooFarFrom(p,q): #takes two complex numbers, returns boolean
...
def isCCW(path, center):
    theta = lambda t: phase(path.point(t) - center) #note: phase range is [-pi,pi] (not continuous on negative real axis)
    n = 100
...
def sortRingListByMinR(ring_list):
    return [val for (key,val) in sorted([(ring.minR, ring) for ring in ring_list])]
...
def aveRadiusEnclosed(path, center):#THIS IS NOT FINISHED
...
#    rad = lambda t: sqrt((x(t)-center.real)**2+(y(t)-center.imag)**2)
    ...
    for seg in path:
        if isinstance(seg,CubicBezier):
            ...
            y = lambda t: b_0*(1-t)**3 + 3*b_1*t*(1-t)**2 + 3*b_2*(1-t)*t**2 + b_3*t**3 #imag part of cubic Bezier
...
#            rad = lambda t: sqrt((x(t)-center.real)**2+(y(t)-center.imag)**2)
    ...
    if aveEnclosed_error > 1e-2: #####Tolerance should be based on native res of SVG (what is area of single pixel)
        raise Exception('area may be erroneous')
    return abs(aveEnclosed/areaEnclosed(path))


def aveRadius_path(path, origin): #origin should be a complex number
    """returns the average distance between origin and path"""
    (aveRad,aveRad_error) = (0,0)
    for seg in path:
        if isinstance(seg,CubicBezier):
            (p_0,p_1,p_2,p_3) = (seg.start, seg.control1, seg.control2, seg.end)
            z = lambda t: p_0*(1-t)**3 + 3*p_1*t*(1-t)**2 + 3*p_2*(1-t)*t**2 + p_3*t**3 #real part of cubic Bezier
            dz = lambda t: 3*(p_1-p_0)*(1-t)**2 + 6*(p_2-p_1)*t*(1-t) + 3*(p_3-p_2)*t**2 #derivative of imaginary part of cubic Bezier
            integrand1 = lambda t: abs((z(t)-origin)*dz(t))
            integrand2 = lambda t: abs(dz(t))
            result = quad(integrand1, 0, 1)/quad(integrand2, 0, 1) #could also use seg.lenth() instead of integrating integrand2
        elif isinstance(seg,Line):
    ...
    if aveRad_error >10**(-4): #Tolerance should be based on native res of SVG (what is area of single pixel)
        raise Exception('area may be erroneous')
    return abs(aveRad)
...
#def cderiv(f,z): #I don't think this works... for some reason
#    f_real = lambda z: f(z).real
#    f_imag = lambda z: f(z).imag
...
#Returns (normal_Line,outer_seg,outer_t) where normal_Line is the Line object normal to innerPath beginning at innerPath.point(innerT) and ending at outer_seg.point(outer_t) or, in the case that the normal line never intersects outerPath, returns (False,False,False)
    ...
    if z_deriv == 0:
        ...
        while z_deriv == 0 and k < 100:
            ...
            z_deriv = segDerivative(inner_seg,offset_t) #non-differentiable point encountered
    n_vec = z_deriv.imag - 1j*z_deriv.real  #note this should always be outward normal as all curves are made CCW earlier
    n_vec = n_vec/abs(n_vec) #normalize

    long_mag = 1.1*abs(extremePointInPath(innerPt,outerPath,1)[0]) #find the max length to the outer
    ...
    if intersec:
        ...
        intersec_withInner = sortby(pathXpathIntersections(Path(long_norm_line),Path(inner_seg)),2)
        if len(intersec_withInner) > 1 and intersec_withInner[1][2]<intersec[0][2]:
            if debug == ('debug',):
                return long_norm_line, False, False
            else:
                return False, False, False
                # intersec_withInner = sortby(pathXpathIntersections(Path(long_norm_line),inner_seg),2)
        ...
        if not isApproxClosedPath(Path(inner_seg)):
            ...
            closingLine_self_intersections = pathXpathIntersections(Path(inner_seg),Path(innerClosingLine))
            skip = False
            for inters in closingLine_self_intersections:
                pt = inters[0].point(inters[2])
                if (not isNear(pt,inner_seg.start)) and (not isNear(pt,inner_seg.end)):
                    skip = True
            if not skip:
                intersec_withClosingLine = sortby(pathXpathIntersections(Path(long_norm_line),Path(innerClosingLine)),2)
                if (intersec_withClosingLine and
                    intersec_withClosingLine[0][2] < intersec[0][2] and
                    not isNear(innerPt, intersec_withClosingLine[0][1].point(intersec_withClosingLine[0][3]))):
        ...
        #this tests if the normal line goes through the center (roughly speaking) and finds a false intersection with outer on the other side of the cross-section
        if not isApproxClosedPath(outerPath):
            outerClosingLine = Line(outerPath.point(0),outerPath.point(1))
            closingLine_self_intersections = pathXpathIntersections(Path(inner_seg),Path(outerClosingLine))
            skip = False
            for inters in closingLine_self_intersections:
                pt = inters[0].point(inters[2])
                if (not isNear(pt,outerPath[0].start)) and (not isNear(pt,outerPath[-1].end)):
                    skip = True
            if not skip:
                intersec_withClosingLine_out = sortby(pathXpathIntersections(Path(long_norm_line),Path(outerClosingLine)),2)
                if len(intersec_withClosingLine_out) > 0 and intersec_withClosingLine_out[0][2]<intersec[0][2]:
                    if debug == ('debug',):
                        return long_norm_line, False, False
                    else:
                        return False, False, False
...
def transect_from_angle(angle, startPoint, outerPath, *debug):
    ...
    if intersec:
        ...
        if debug == ('debug',):
            ...
            return False, False, False
# def boundingBoxOfPaths(pathStringList):
#     from scipy.optimize import fmin_cobyla
#     xmin_list, xmax_list,ymin_list,ymax_list = [],[],[],[]
#     for ps in pathStringList:
#         p = parse_path(ps)
#         xmin_list.append(p.point(float(fmin_cobyla(lambda t: p.point(t).real, .5, [lambda t: t,lambda t:1-t], rhoend=1e-7,iprint=0))).real)
#         xmax_list.append(p.point(float(fmin_cobyla(lambda t: -p.point(t).real, .5, [lambda t: t,lambda t:1-t], rhoend=1e-7,iprint=0))).real)
#         ymin_list.append(p.point(float(fmin_cobyla(lambda t: p.point(t).imag, .5, [lambda t: t,lambda t:1-t], rhoend=1e-7,iprint=0))).imag)
#         ymax_list.append(p.point(float(fmin_cobyla(lambda t: -p.point(t).imag, .5, [lambda t: t,lambda t:1-t], rhoend=1e-7,iprint=0))).imag)
#     return min(xmin_list), max(xmax_list), min(ymin_list), max(ymax_list)

def displaySVGPaths_numbered(pathList,savefile,*colors): #creates and saves an svf file displaying the input paths
    import svgwrite
...
def displaySVGPaths_named(pathList,name,*colors): ###DEPRECATED #creates and saves an svf file displaying the input paths
    import svgwrite
    filename = name+'.svg'
    dwg = svgwrite.Drawing(filename, size=('2000px','2000px'), viewBox="0 0 2000 2000")
    dwg.add(dwg.rect(insert=(0, 0), size=('100%', '100%'), rx=None, ry=None, fill='white')) #add white background
    dc = 100/len(pathList)
    for i,p in enumerate(pathList):
        if isinstance(p, Path):
...
#returns (|ring.seg.point(t)-pt|,t,seg,ring,i) where t minimizes the distance between pt and curve ring.point(t) for 0<=t<=1 and ring the closest ring to pt in ring_list; i is the index of ring in ring_list
    result_list = []
...
#made to sort T-values so that they proceed backwards (CCW assuming CW path parameterization) from Tstar
#creates an object with a method distfcn that returns the theta-distance that (traveling backwards) input is from Tstar.  This can be used as a key fcn to sort.
