...
def rotate(curve, degs, origin=None):
    ...
    if isinstance(curve, Path):
        return Path(*[transform(seg, degs, origin=origin)])
    elif is_bezier_segment(curve):
        return bpoints2bezier([transform(bpt) for bpts in bez.bpoints()])
...
# def affine(curve, A, b):
#     """Returns the curve after passing it through the affine
#     transformation Ax+b."""
#     if isinstance(curve, Path):
#         return Path(*[trans(seg, A, b)])
#     elif is_bezier_segment(curve):
#         X = matrix([[z.real, z.imag] for z in curve.bpoints()]).T
#         new_bpts = matrix(A)*X + matrix(b)
#         return bpoints2bezier([complex(xy[0], xy[1]) for xy in new_bpts])
#     elif isinstance(curve, Arc):
...
def translate(curve, z0):
    ...
    """
    if isinstance(curve, Path):
        return Path(*[translate(seg)])
    elif is_bezier_segment(curve):
        return bpoints2bezier([bpt + z0 for bpts in bez.bpoints()])
    elif isinstance(curve, Arc):
...
def segment_length(curve, start, end, start_point, end_point, error, min_depth,
                   depth):
...
class Arc(object):
    def __init__(self, start, radius, rotation, large_arc, sweep, end,
        ...
        sweep : bool
            ...
            The end point of the large_arc.
    ...
    def __repr__(self):
        return ("Arc(start=%s, radius=%s, rotation=%s, "
                "large_arc=%s, sweep=%s, end=%s)"
             .format(self.start, self.radius, self.rotation,
    ...
    def _parameterize(self):
        ...
        # The center in (x,y) coordinates is easy to find knowing (c_x', c_y')
