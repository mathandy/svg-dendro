...
from options4rings import stroke_width_default, tol_intersections, try_to_open_svgs_in_browser, rings_may_contain_unremoved_kinks
...
def isClosedPathStr(pathStr): #takes in SVF 'd=' path string and outputs true if closed (according to SVG)
    tmp = pathStr.rstrip()
...
def cubicCurve(P,t): #Evaluated the cubic Bezier curve (given by control points P) at t
...
#    return [complex(z[0]) for z in array(res)]
        ...
        P=cub
    return P[0], -3*P[0] + 3*P[1], 3*P[0] - 6*P[1] + 3*P[2], -P[0] + 3*P[1] - 3*P[2] + P[3]
...
def reverseSeg(seg): #reverses the orientation of a Line or CubicBezier segment
...
#        raise Exception("Input argument must be a Line object nor a CubicBezier object.")
    return seg.reversed()
def reversePath(path): #reverses the orientation of a path object composed of lines and cubicBeziers segments
    newpath = []
    for seg in path:
...
#returns the path string for the line connecting these two points
    return 'M'+str(p0.real)+' '+str(p0.imag)+'L'+str(p1.real)+' '+str(p1.imag)
def cubicPoints2String(P):
    return "M %s,%s C %s,%s %s,%s %s,%s"%(P[0].real,P[0].imag,P[1].real,P[1].imag,P[2].real,P[2].imag,P[3].real,P[3].imag)
def cubicPoints2RelString(P):
    rP = [p-P[0] for p in P]
    return "M %s,%s c %s,%s %s,%s %s,%s"%(P[0].real,P[0].imag,rP[1].real,rP[1].imag,rP[2].real,rP[2].imag,rP[3].real,rP[3].imag)
...
#returns (|path.seg.point(t)-pt|,t,seg) where t minimizes/maximizes the distance between pt and curve path.point(t) #returns abs of area enclosed by (closed) curve path.point(t) for t in [0,1]
    if min_or_max not in {0,1}:
        raise Exception("min_or_max must be 0 or 1.\n 0 if minimizer is desired, 1 if maximizer is desired.")
    p,q = pt.real,pt.imag
    current_extremizer = None ###should be based on native res
    for seg in path:
        if isinstance(seg,CubicBezier):
            (a_0,a_1,a_2,a_3) = (seg.start.real, seg.control1.real, seg.control2.real, seg.end.real)
            (b_0,b_1,b_2,b_3) = (seg.start.imag, seg.control1.imag, seg.control2.imag, seg.end.imag)
            dist = lambda t: ((a_0*(t - 1)**3 - 3*a_1*(t - 1)**2*t + 3*a_2*(t - 1)*t**2 - a_3*t**3 + p)**2 + (b_0*(t - 1)**3 - 3*b_1*(t - 1)**2*t + 3*b_2*(t - 1)*t**2 - b_3*t**3 + q)**2)**(0.5) #gives the distance from pt to path.point(t)
            polycoeffs_dist_deriv = [6*(a_0 - 3*a_1 + 3*a_2 - a_3)**2 + 6*(b_0 - 3*b_1 + 3*b_2 - b_3)**2, -30*(a_0 - 2*a_1 + a_2)*(a_0 - 3*a_1 + 3*a_2 - a_3) - 30*(b_0 - 2*b_1 + b_2)*(b_0 - 3*b_1 + 3*b_2 - b_3), 36*(a_0 - 2*a_1 + a_2)**2 + 24*(a_0 - a_1)*(a_0 - 3*a_1 + 3*a_2 - a_3) + 36*(b_0 - 2*b_1 + b_2)**2 + 24*(b_0 - b_1)*(b_0 - 3*b_1 + 3*b_2 - b_3), -54*(a_0 - a_1)*(a_0 - 2*a_1 + a_2) - 6*(a_0 - 3*a_1 + 3*a_2 - a_3)*(a_0 - p) - 54*(b_0 - b_1)*(b_0 - 2*b_1 + b_2) - 6*(b_0 - 3*b_1 + 3*b_2 - b_3)*(b_0 - q), 18*(a_0 - a_1)**2 + 12*(a_0 - 2*a_1 + a_2)*(a_0 - p) + 18*(b_0 - b_1)**2 + 12*(b_0 - 2*b_1 + b_2)*(b_0 - q), -6*(a_0 - a_1)*(a_0 - p) - 6*(b_0 - b_1)*(b_0 - q)] #coefficients for polynomial (d/dt)dist = (d/dt)|path.point(t)-pt|)
            possible_extremizers = list(roots(polycoeffs_dist_deriv)) + [0,1]
            for t in possible_extremizers:
                if t.real == t and 0<=t<=1:
                    t=t.real
                    try:
                        if  dist(t)*(-1)**min_or_max < current_extremizer[0]*(-1)**min_or_max :
            ...
            polycoeffs_dist_deriv = [2*(a_0 - a_1)**2 + 2*(b_0 - b_1)**2, -2*(a_0 - a_1)*(a_0 - p) - 2*(b_0 - b_1)*(b_0 - q)]
            dist = lambda t: ((a_0*(t - 1) - a_1*t + p)**2 + (b_0*(t - 1) - b_1*t + q)**2)**(0.5)
            possible_extremizers = list(roots(polycoeffs_dist_deriv)) + [0,1]
            for t in possible_extremizers:
                if t.real == t and 0<=t<=1:
                    t=t.real
...
def normal(path_or_seg,T_or_t):
    ...
    return n/abs(n)
def segCurvature(seg,t,use_limit_for_zero_divs = True):  #This returns the curvature of the segment at t
#    if isinstance(seg,CubicBezier):
#        P = (seg.start,seg.control1,seg.control2,seg.end)
...
def curvature(path_or_seg,T_or_t,check_for_kinks=check_for_kinksTrue):
    ...
    if check_for_kinks and isinstance(path_or_seg,Path):
        ...
        if isclose(t,0) and (seg_idx!=0 or isClosed(path)):
            if not isclose(normal(seg,0),normal(path[(seg_idx-1)%len(path)],1)):
                return inf
        elif isclose(t,1) and (seg_idx!=len(path)-1 or isClosed(path)):
            if not isclose(normal(seg,1),normal(path[(seg_idx+1)%len(path)],0)):
                return inf
    if isinstance(path_or_seg,Line):
        ...
        raise Exception("First argument must be a Line, CubicBezier, or Path containing only Lines and CubicBeziers.")
...
def printPath(path):
    ...
    if not isinstance(path,Path):
        if isinstance(path,Line) or isinstance(path,CubicBezier):
            path = Path(path)
        elif all([isinstance(seg,Line) or isinstance(seg,CubicBezier) for seg in path]):
            path = Path(*path)
        else:
    ...
    for seg_index_, seg in enumerate(path):
        ...
        if isinstance(seg,CubicBezier):
            ...
            for z in cubPoints(seg):
                tmp += [z.real,z.imag]
            nicePts = "(%.1f + i%.1f, %.1f + i%.1f, %.1f + i%.1f, %.1f + i%.1f)"%tuple(tmp)
            output_string+= "[%s] - CubicBezier: "%seg_index_ + nicePts
        elif isinstance(seg,Line):
            nicePts = "(%.1f + i%.1f, %.1f + i%.1f)"%(seg.start.real,seg.start.imag,seg.end.real,seg.end.imag,)
    ...
    if path[0].start == path[-1].end:
        ...
        closure = "Open  "
    output_string += "\n"+"[*] "+closure+" : |path.point(0) - path.point(1)| = %s"%abs(path.point(0) - path.point(1))
    print(output_string)
...
def areaEnclosed(path, closure_tolerance=opt.tol_isApproxClosedPath):
    ...
    if not path.isclosed() and abs(path[0].start - path[-1].end) >= closure_tolerance:
    ...
    for seg in path:
        if isinstance(seg, CubicBezier):
            ...
            x = lambda t: a_0*(1-t)**3 + 3*a_1*t*(1-t)**2 + 3*a_2*(1-t)*t**2 + a_3*t**3  # real part of cubic Bezier
            dy = lambda t: 3*(b_1-b_0)*(1-t)**2 + 6*(b_2-b_1)*t*(1-t) + 3*(b_3-b_2)*t**2  # derivative of imaginary part of cubic Bezier
...
def pathXpathIntersections(path1, path2, justonemode=False, tol=1e-4):
    ...
    return [(inter[0][1], inter[1][1], inter[0][2], inter[1][2]) for inter in path1.intersect(path2)]
...
def path_listXpathIntersections(path_list,path):# returns list of lists of tuples (seg,seg_o,t1,t2)
...
def pathXlineIntersections(inline,path):
    ...
    for seg in path:
        if isinstance(seg,CubicBezier):
            ...
            raise Exception("seg is not in a Line object or Cubic Bezier object.")
    return intersection_list
...
#Takes in list of paths and returns list of tuples (tl,path_index,seg,tp) where line.point(tl) = seg.point(tp) where seg is a segment in path_list[index]
    return [(tl,path_index,seg,tp) for path_index,path in enumerate(path_list) for (tl,seg,tp) in pathXlineIntersections(inline,path)]
...
#the method here is to rotate the line to be the x-axis (and the cubic with it), then the intersection points are just the roots of the rotated cubic.
    if line.start==line.end:
        return []
    lP0,lP1 = array([line.start.real,line.start.imag]), array([line.end.real,line.end.imag])
    cP0,cP1,cP2,cP3 = array([cub.start.real,cub.start.imag]),array([cub.control1.real,cub.control1.imag]),array([cub.control2.real,cub.control2.imag]),array([cub.end.real,cub.end.imag])
    lQ0,lQ1,cQ0,cQ1,cQ2,cQ3 = array([0,0]),lP1-lP0,cP0-lP0,cP1-lP0,cP2-lP0,cP3-lP0 #translates all points so that the line goes through the origin (with P0=0)
    a = lQ1[0]; b=lQ1[1]
    if a:
        R = array([[1/a,0],[-b/a,1]]) #inverse of [[a,0],[b,1]]
    else:
        R = array([[0, 1/b],[1,-a/b]]) #inverse of [[a,1],[b,0]]
    lQ1,cQ0,cQ1,cQ2,cQ3 = dot(R,lQ1),dot(R,cQ0),dot(R,cQ1),dot(R,cQ2),dot(R,cQ3) #rotates all points so that the line goes along the x-axis the origin (with P0=(0,0) and P1 = (1,0))
    ...
    coeffs_y = [-b_0 + 3*b_1 - 3*b_2 + b_3, 3*b_0 - 6*b_1 + 3*b_2, -3*b_0 + 3*b_1, b_0]
    roots_y = list(roots(coeffs_y))
#    roots_y = list(cubroots_nr(coeffs_y)) #numpy newton-ralphon tests as slower, so using numpy roots
    a = (cQ0[0],cQ1[0],cQ2[0],cQ3[0]) #to find x-values associated with y(t)=0
    intersection_list = []
    for r in set(roots_y):
        if not r.imag and 0<= r<= 1:
            x_val = cubicCurve(a,r)
            if 0<=x_val<=1:
                intersection_list.append((r,x_val)) #note, after rotation line is parameterized by x(t)=t and y(t)=0
...
def lineXlineIntersections(seg1,seg2):
    ...
    c = [z.real for z in q]
    d = [z.imag for z in q]
    denom  = (b[0] - b[1])*c[0] - (b[0] - b[1])*c[1] - a[0]*(d[0] - d[1]) + a[1]*(d[0] - d[1])
    if denom==0:
        return []
    t1 = ((b[0] - d[1])*c[0] - (b[0] - d[0])*c[1] - a[0]*(d[0] - d[1]))/denom
    t2 = -(a[1]*(b[0] - d[0]) - a[0]*(b[1] - d[0]) - (b[0] - b[1])*c[0])/denom
...
def cubicMinMax_x(points):
    ...
    if denom != 0:
        ...
        dcoeffs.reverse()
        dcoeffs = dcoeffs[0:-1]
        local_extremizers += polyroots(dcoeffs,condition=lambda r: 0<r<1,realroots=True)
...
def trimSeg(seg,t0,t1):
    ...
    if isinstance(seg,CubicBezier):
        ...
        raise Exception("Segment sent to trimSeg is neither Line object nor CubicBezier obect.")
    if t0==0:
...
#    path = parse_path(path2str(path)) ###DEBUG (maybe can remove if speed demands it)
    ...
    if T0<T1 and i0==i1:
        ...
        if T1==T0:
            ...
            if not path.closed():
                raise Exception("T1<T0 and path is open.  I think that means you put in the wrong T values.")
            else:
...
def boundingBox(curves):
    ...
    elif all([isinstance(c,Line) or isinstance(c,CubicBezier) or isinstance(c,Path) for c in curves]):
        bblist = [boundingBox(c) for c in curves]
        return bigBoundingBox(bblist)
    else:
        raise Exception("Input argument should be a Path or Line or CubicBezier or a list of such objects.")

def path2str(path, closure_tolerance=0):
    d = "M %s,%s"%(z2xy(path[0].start))
    for seg in path[0:len(path)-1]:
        if isinstance(seg,CubicBezier):
            d += " C %s,%s %s,%s %s,%s"%(z2xy(seg.control1)+z2xy(seg.control2)+z2xy(seg.end))
        elif isinstance(seg,Line):
    ...
    if isinstance(lastseg,CubicBezier):
        d += " C %s,%s %s,%s %s,%s"%(z2xy(lastseg.control1)+z2xy(lastseg.control2)+z2xy(lastseg.end))
...
def disvg(paths,colors= [],nodes=None,node_colors=None,node_radii=None,
          lines=None,line_colors=None,filename=os_path.join(getcwd(),'temporary_displaySVGPaths.svg'),
          openInBrowser=False,stroke_width=stroke_width_default,margin_size=0.1):
    ...
    for i,p in enumerate(paths):
        if isinstance(p,Path):
            ...
            ps = p
        dwg.add(dwg.path(ps, stroke=colors[i], stroke_width=str(stroke_width), fill='none'))
    #add lines
    if lines != None:
        if line_colors==None:
            line_colors=["#0000ff"]*len(lines)
        for i,l in enumerate(lines):
            start=(l.start.real,l.start.imag); end=(l.end.real,l.end.imag)
            dwg.add(dwg.line(start=start,end=end, stroke=line_colors[i], stroke_width=str(stroke_width), fill='none'))
    ...
    if nodes != None:
        ...
        for i_pt,pt in enumerate([(z.real,z.imag) for z in nodes]):
            dwg.add(dwg.circle(pt,node_radii[i_pt], stroke=node_colors[i_pt], fill=node_colors[i_pt]))
...
def svgSlideShow(pathcolortuplelist,save_directory=None,clear_directory=False,suppressOutput=False):
    ...
    if clear_directory: #clear all files in slideshow folder
        for the_file in listdir(save_directory):
            ...
            if os_path.isfile(file_path): #make sure file is a file (not subfolder)
    ...
    output2file('mogrify -format jpg *.svg',filename=os_path.join(save_directory,"convertSVGs2jpgs.bat"),mode='w')
...
def svg2pathlist(SVGfileLocation):
    ...
    path_strings += [polylineStr2pathStr(p.getAttribute('points')) for p in doc.getElementsByTagName('polyline')]
    #Use minidom to extract polygon strings from input SVG, convert to path strings, add to list
    path_strings += [polylineStr2pathStr(p.getAttribute('points'))+'z' for p in doc.getElementsByTagName('polygon')]
    ...
    path_strings += ['M' + p.getAttribute('x1') + ' ' +p.getAttribute('y1') + 'L'+p.getAttribute('x2') + ' ' + p.getAttribute('y2') for p in doc.getElementsByTagName('line')]
    doc.unlink()
